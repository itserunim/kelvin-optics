  ...
  return (
    <div className="min-h-screen bg-[#e0e5ec] text-slate-700 font-sans p-6">
      <div className="flex items-center gap-4 mb-8">
        <Link
          href="/"
          className="w-12 h-12 flex items-center justify-center bg-[#e0e5ec] rounded-full shadow-[6px_6px_12px_#a3b1c6,-6px_-6px_12px_#ffffff] active:shadow-[inset_4px_4px_8px_#a3b1c6,inset_-4px_-4px_8px_#ffffff] transition-all text-slate-500 hover:text-slate-700"
          aria-label="Go back"
        >
        ...
        <h1 className="text-3xl font-extrabold text-slate-500">Dashboard</h1>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
        ...
        <NeuCard className="p-6 md:col-span-2 flex justify-between items-center">
          <div>
            ...
            <p className={`font-semibold ${sensors.displayOn ? "text-green-500" : "text-red-400"}`}>
              {sensors.displayOn ? "Active" : "Sleep Mode"}
            </p>
          </div>
          <div className="text-right">
            ...
            <p className="font-bold text-slate-600 text-xl">
              {sensors.currentSlide === 0 ? "Temp" : sensors.currentSlide === 1 ? "Time" : "Pressure"}
            </p>
          </div>
        </NeuCard>
      </div>
